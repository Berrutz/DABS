...
## Macchina A – 100.111.85.92 (JADE Main)
...
export PUBLIC_HOST=100.111.85.92
docker compose --profile main up -d jade-main
```

- Monitor: `http://100.111.85.92:4100/monitor`
...
## Macchina B – 100.76.138.54 (user-ui remoto)
...
$env:PUBLIC_HOST = "100.76.138.54"   # IP Tailscale pubblicato nei riferimenti RMI
$env:MAIN_HOST   = "100.111.85.92"   # indirizzo del Main a cui collegarsi
$env:QUERY_HOST  = "100.111.85.92"   # la UI invia le query al QueryAgent remoto
$env:LOCAL_HOST  = "100.76.138.54"   # IP su cui il CommandDispatcher (JICP) deve bindare
...
export PUBLIC_HOST=100.76.138.54
export MAIN_HOST=100.111.85.92
export QUERY_HOST=100.111.85.92
export LOCAL_HOST=100.76.138.54
...
- GUI via Tailscale: `http://100.76.138.54:4000`
...
- `-host 100.111.85.92` – indirizzo del Main (IP Tailscale della macchina A).
- `-port 1099` – porta JADE IMTP/RMI del Main.
- `-local-host 100.76.138.54` – (quando impostato) pubblica verso il Main l'IP Tailscale reale della macchina B; senza questo, JADE sceglierebbe l'IP del bridge Docker e il Main genererebbe l'errore `DispatcherException`/`No skeleton`.
...
- `-Djava.rmi.server.hostname=100.76.138.54` – forza gli stub RMI (usati dai servizi ausiliari) a pubblicare l'IP Tailscale, evitando riferimenti allo spazio indirizzi interno del container.
...
- Da B esegui `Test-NetConnection -ComputerName 100.111.85.92 -Port 1099` (o `nc -vz 100.111.85.92 1099`) per verificare la raggiungibilità del Main.
